...
public class BasicItemSearch implements ItemSearch {
    ...
    @Getter private final Map<SearchFilter, String> parameters;

    public BasicItemSearch() {
        this(new HashMap<>());
    }

    public BasicItemSearch(Map<SearchFilter, String> parameters) {
    ...
    public static BasicItemSearch fromQuery(String query) {
        if (!query.contains(":")) {
            return new BasicItemSearch(Map.of(BasicItemSearch.SearchFilter.NAME, query));
        }
        ...
        return new BasicItemSearch(options);
    }
